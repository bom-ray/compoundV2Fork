...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function redeemVerify(address cToken, address redeemer, uint redeemAmount, uint redeemTokens) pure override external {
        // Shh - currently unused
        cToken;
        redeemer;
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) override external returns (uint) {
        ...
        if (!markets[cToken].accountMembership[borrower]) {
            ...
            Error errOne = addToMarketInternal(CToken(msg.sender), borrower);
            if (errOne != Error.NO_ERROR) {
                return uint(errOne);
            }
    ...
    function liquidateBorrowAllowed(
        ...
        address liquidator,
        address borrower,
        uint repayAmount) view override external returns (uint) {
        // Shh - currently unused
        liquidator;
